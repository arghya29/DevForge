...
HTML-escapes a string for safe syntax-highlighter injection, including
backticks used by template-literal highlighting.

### `escapeHtml(s: string): string`

HTML-escapes console text for safe `innerHTML` rendering. Unlike `escHtml()`,
it intentionally leaves backticks unchanged because console output is not
reprocessed by the template-literal highlighter.
